...
const App = () => {
	...
	React.useEffect(() => {
		fetch('/networks')
			...
			.catch((e: Error) => {
				setError(`Verfügbare Netzwerke konnten nicht abgerufen werden.${e.message || e}`);
			})
			.finally(() => {
	...
	return (
		<Provider>
			<GlobalStyle />
			<Navbar style={{ backgroundColor:"#194154"}} brand={<img src={logo} style={{ height: 30}} alt="logo" />} />
